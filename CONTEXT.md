...
- Hoist OpenAI `system` messages into Claude Code's `--system-prompt`; do not duplicate them inside the rendered conversation transcript.
...
- OpenAI Python retries twice by default. Example clients set `max_retries=0` so one logical turn cannot silently create a duplicate Claude session.
